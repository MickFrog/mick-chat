  ...
  useEffect(() => {
    const fetchChats = () => {
      const unsub = onSnapshot(
        doc(fireDB, "userChats", currentUser.uid),
        (doc) => {
          setChats(doc.data());
        }
      );
      ...
      return () => {
        //clean up resources of snapshot listener
        unsub();
      };
    };
  ...
  return (
    <div className="flex flex-col overflow-y-auto my-custom-scrollbar">
      {Object.entries(chats)
        ?.sort((a, b) => b[1].date - a[1].date) //convert object to array + sort starting from latest date
        .map((chatInfo) => {
