...
const SearchModal = (props) => {
  ...
  const handleSearch = () => {
    //filter users according to user search username
    if (searchInput.value === "") return;

    setSearchedUsers(
      users.current.filter((elem) =>
        elem.displayName.toLowerCase().includes(searchInput.value.toLowerCase())
      )
    );
  };
  ...
  const handleUserSelect = async (user) => {
    ...
    try {
      ...
      if (!docSnapshot.exists()) {
        ...
        await updateDoc(doc(fireDB, "userChats", user.userID), {
          //for other user
          [combinedId + ".userInfo"]: {
          ...
          {searchedUsers.length !== 0 &&
            searchedUsers.map((userDoc) => {
              return (
                <div
                  key={userDoc.userID}
                  className="rounded flex gap-4 items-center p-2 hover:bg-neutral-500 hover:bg-opacity-70 hover:cursor-pointer"
